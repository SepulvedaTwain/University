...
int main(int argc, char *argv[]){
	...
	int server_port=argvCheck(argc,argv);
	if(server_port==0){
...
void * LogOn_Thread(void *ptr){
	...
	if(!strcmp(token,"LOG_ON")){
		...
		r=read(socket,buf,1000);
		prepare_GetList(buf2,clientIP,port);
		write(socket,buf2,1000);
		...
		if(!clients.findNode(clientIP,port)){
			...
			write(socket,"ERROR_IP_PORT_NOT_FOUND_IN_LIST ",1000);
			close(socket);
		}
		else{
...
void prepare_GetList(char * buf,char * clientIP,unsigned short port){
	...
	while(temp){
		if(temp->active && (strcmp(temp->ip_address,clientIP) || temp->port!=port ) ){
			...
			if(buf[0]==0){
				sprintf(buf3,"%s %s ",temp->ip_address,b);//edw den einai lathos 
				strcat(buf2,buf3);
			}
			else{
				sprintf(buf2,"%s%s %s ",buf,temp->ip_address,b);
				strcat(buf2,buf3);
			}
	...
	sprintf(buf,"CLIENT_LIST %d %s ",counter,buf2);
	cout<<buf<<endl;
}
