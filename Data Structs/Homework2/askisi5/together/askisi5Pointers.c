...
void BHRemove(BHHeap * heap){
	...
	BHNode * temp=CBTGetLast(heap);//pernoume ton last node
	...
	temp=heap->root;
	while(1){//ton allazoume me to root kai meta ton katebazoume oso xreiastei
...
void BHInsert(BHHeap * heap ,int priority ,BHItem item){
	...
	CBTInsertLast(heap,item,priority);//bazoume to node sto telos
	BHNode * temp=CBTGetLast(heap);
	while(temp->parent){//kai meta to anebazoume oso xreiastei
...
void Heapify2Nodes(BHNode * parent,BHNode * child){//allazei dio nodes
...
void Heapify(BHNode * root, int level, int counter){
	...
	if(root->right==NULL){//allazei dio node
	...
	if(root->right->priority > root->priority || root->left->priority > root->priority){
		...
		Heapify2Nodes(root,temp);//allazei dio node
	}
...
BHHeap * BHHeapify(int size, int * priorities, BHItem * items){
	//fitaxnoume to dentro kai kaloume tin heapify
