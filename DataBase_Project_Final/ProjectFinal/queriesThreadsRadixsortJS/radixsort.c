...
void radixSort( relation *rel1, relation *rel2, int start, int end, int numByte, int currentArray)
{
	...
	if ( (((end-start)*sizeof(tuple)+1) > (1024*64)) && (numByte < 9) )
	{
		...
		for ( i=0; i< (2*2*2*2*2*2*2*2); i++)
		{
			if ( i != (2*2*2*2*2*2*2*2) - 1)
			{
				if ( psum[i] != psum[i+1])
				{
					jobscheduler( rel2, rel1, psum[i], psum[i+1] - 1, numByte+1, currentArray, false);
				}
			}
			else
			{
				if ( psum[i] <= end)
				{
					jobscheduler( rel2, rel1, psum[i], end, numByte+1, currentArray, false);
				}
			}
		}
	...
	{
		//jobscheduler( rel1, rel2 , start, end, -1, -1, true );
